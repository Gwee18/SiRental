...
@section('content')
        ...
        <div class="relative z-10 max-w-7xl mx-auto px-6 w-full">
            <div class="max-w-2xl">
                ...
                <p class="text-white/80 text-lg leading-relaxed mb-10">
                    Perlengkapan mendaki gunung berkualitas, bersih, dan terawat untuk petualangan Anda. Kami memastikan setiap alat siap tempur untuk puncak impian Anda.
                </p>
                <div class="flex flex-wrap gap-4">
                    @auth('web')
                        <a href="#katalog" class="bg-[#68dbae] hover:bg-[#55c99c] text-[#00372c] font-semibold px-8 py-3.5 rounded-xl transition-all duration-200 text-sm">
                            Rental Sekarang
                        </a>
                    @else
                        <a href="#katalog" class="bg-[#68dbae] hover:bg-[#55c99c] text-[#00372c] font-semibold px-8 py-3.5 rounded-xl transition-all duration-200 text-sm">
    ...
    {{-- STATS BAR --}}
    <section class="bg-[#085041] text-white py-12">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            <div class="space-y-1">
                <div class="text-4xl font-bold text-[#68dbae]">{{ $totalAlat }}+</div>
                <div class="text-sm text-white/70 uppercase tracking-widest font-medium">Alat Tersedia</div>
            </div>
            <div class="space-y-1 md:border-x border-white/10 md:py-0 py-4 border-y md:border-y-0">
                <div class="text-4xl font-bold text-[#68dbae]">{{ $totalCustomer }}+</div>
                <div class="text-sm text-white/70 uppercase tracking-widest font-medium">Pelanggan Puas</div>
            </div>
            <div class="space-y-1">
                <div class="text-4xl font-bold text-[#68dbae]">4.9</div>
                <div class="text-sm text-white/70 uppercase tracking-widest font-medium">Rating Google</div>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="text-center space-y-4">
                    <div class="w-16 h-16 rounded-2xl bg-[#e8f5f0] flex items-center justify-center mx-auto">
                        <svg width="28" height="28" fill="none" stroke="#085041" stroke-width="1.8" viewBox="0 0 24 24">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed">Buat akun untuk mulai memantau ketersediaan perlengkapan secara real-time.</p>
                </div>
                ...
                <div class="text-center space-y-4">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed">Pilih kategori alat yang dibutuhkan sesuai durasi pendakian Anda.</p>
                </div>
                ...
                <div class="text-center space-y-4">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed">Lengkapi detail peminjaman dan unggah identitas untuk jaminan keamanan.</p>
                </div>
                ...
                <div class="text-center space-y-4">
                    ...
                    <p class="text-gray-500 text-sm leading-relaxed">Tim kami akan memverifikasi dan alat siap Anda ambil di gerai kami.</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-md hover:-translate-y-1 transition-all duration-200">
                    ...
                    <div class="p-5 space-y-3">
                        <div>
                            <span class="text-xs text-[#085041] font-semibold uppercase tracking-wide">{{ $item->kategori }}</span>
                            <h4 class="font-semibold text-[#00372c] text-base mt-0.5">{{ $item->nama_alat }}</h4>
                        </div>
                        <p class="text-[#085041] font-bold text-base">
                            Rp {{ number_format($item->harga_per_hari, 0, ',', '.') }} <span class="text-gray-400 font-normal text-sm">/ hari</span>
                        </p>
                        <div class="flex items-center justify-between pt-1">
                            ...
                            @if($item->stok_tersedia > 0)
                                <a href="#harga" class="text-[#085041] hover:text-[#00372c] transition-colors">
                    ...
                    <table class="w-full bg-white border border-gray-100 rounded-b-xl overflow-hidden">
                        ...
                        <tbody class="text-sm text-gray-700">
                            ...
                            <tr class="border-t border-gray-100 hover:bg-gray-50 transition-colors {{ $i % 2 == 0 ? 'bg-white' : 'bg-gray-50/50' }}">
                                <td class="px-6 py-4 font-medium text-[#00372c]">{{ $item->nama_alat }}</td>
                                <td class="px-6 py-4 text-center">Rp {{ number_format($item->harga_per_hari * 1, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-center">Rp {{ number_format($item->harga_per_hari * 2, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-center">Rp {{ number_format($item->harga_per_hari * 3, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-center">Rp {{ number_format($item->harga_per_hari * 4, 0, ',', '.') }}</td>
                                <td class="px-6 py-4 text-center">Rp {{ number_format($item->harga_per_hari * 5, 0, ',', '.') }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                ...
                <div class="relative">
                    ...
                    >
                    <div class="absolute -bottom-6 -right-6 bg-[#085041] text-white p-6 rounded-2xl shadow-xl hidden lg:block">
                        <div class="text-3xl font-bold text-[#68dbae]">5+</div>
                        <div class="text-sm text-white/70 mt-1">Tahun Pengalaman</div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <h2 class="text-3xl font-bold text-[#00372c] mt-2 mb-4">Mitra Terpercaya Para Pendaki Indonesia</h2>
                        <p class="text-gray-500 leading-relaxed text-base">
                            SiRental hadir sebagai solusi bagi para pendaki yang ingin menikmati alam tanpa harus membeli perlengkapan mahal. Kami menyediakan alat pendakian berkualitas tinggi yang terawat dan siap digunakan.
                        </p>
                    </div>

                    <div class="space-y-4">
                        <div class="flex items-start gap-4">
                            <div class="w-10 h-10 rounded-xl bg-[#e8f5f0] flex items-center justify-center shrink-0">
                                <svg width="20" height="20" fill="none" stroke="#085041" stroke-width="2" viewBox="0 0 24 24">
                                    <polyline points="20 6 9 17 4 12"/>
                                </svg>
                            </div>
                            <div>
                                <h4 class="font-semibold text-[#00372c] text-sm">Alat Terawat & Berkualitas</h4>
                                <p class="text-gray-500 text-sm mt-0.5">Setiap alat dicek dan dibersihkan sebelum dipinjamkan.</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-4">
                            <div class="w-10 h-10 rounded-xl bg-[#e8f5f0] flex items-center justify-center shrink-0">
                            ...
                            <div>
                                ...
                                <p class="text-gray-500 text-sm mt-0.5">Tidak ada biaya tersembunyi, semua harga tertera jelas.</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-4">
                            <div class="w-10 h-10 rounded-xl bg-[#e8f5f0] flex items-center justify-center shrink-0">
                            ...
                            <div>
                                ...
                                <p class="text-gray-500 text-sm mt-0.5">Daftar, pilih alat, dan konfirmasi dalam hitungan menit.</p>
                            </div>
                        </div>
                    </div>

                    <div class="flex flex-wrap gap-4 pt-2">
                        <a href="#katalog" class="bg-[#085041] hover:bg-[#00372c] text-white font-semibold px-6 py-3 rounded-xl transition-all text-sm">
                            Mulai Rental
                        </a>
    ...
    <section class="py-20 bg-[#085041]">
        <div class="max-w-7xl mx-auto px-6 text-center">
            ...
            <p class="text-white/70 text-base mb-8 max-w-xl mx-auto">Daftar sekarang dan nikmati kemudahan rental alat pendakian berkualitas langsung dari genggaman Anda.</p>
            <div class="flex flex-wrap justify-center gap-4">
                <a href="#katalog" class="bg-[#68dbae] hover:bg-[#55c99c] text-[#00372c] font-semibold px-8 py-3.5 rounded-xl transition-all text-sm">
